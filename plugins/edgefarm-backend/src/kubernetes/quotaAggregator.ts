...
export class MemoryQuotaAggregator implements QuotaAggregator {
    fromKubernetesDetailsResponse(data: NodeDetails): QuotaValues {
        ...
        ({ value: result.value, unit: result.unit } = this.convertToHumanReadable(result.raw!));
    ...
    fromKubernetesQuotaResponse(data: any): ResourceQuota {
        ...
        limit.raw = JSONPath({ path: '$.items..resources.limits.memory', json: data })
            .map(this.convertToValueWithoutUnit)
            .reduce((partialSum: number, a: number): number => partialSum + a, 0);
        request.raw = JSONPath({ path: '$.items..resources.requests.memory', json: data })
            .map(this.convertToValueWithoutUnit)
            .reduce((partialSum: number, a: number): number => partialSum + a, 0);

        ({ value: limit.value, unit: limit.unit } = this.convertToHumanReadable(limit.raw!));
        ({ value: request.value, unit: request.unit } = this.convertToHumanReadable(request.raw!));
    ...
    private convertToHumanReadable(value: number): { value: number, unit: string } {
...
export class CpuQuotaAggregator implements QuotaAggregator {
    ...
    fromKubernetesQuotaResponse(data: any): ResourceQuota {
        ...
        limit.raw = JSONPath({ path: '$.items..resources.limits.cpu', json: data })
            .map(this.convertToValueWithoutUnit)
            .reduce((partialSum: number, a: number): number => partialSum + a, 0);
        request.raw = JSONPath({ path: '$.items..resources.requests.cpu', json: data })
            .map(this.convertToValueWithoutUnit)
            .reduce((partialSum: number, a: number): number => partialSum + a, 0);
    ...
    private convertToHumanReadable(value: number): { value: number, unit: string } {
        if (Math.floor(value) === value) return { value: value, unit: value > 1 ? ' cores' : ' core' }
